...
{
public:
	GE_INLINE Condition(Mutex * mutex) : m_nLockCount(0), m_externalMutex(mutex)
	{
	...
	GE_INLINE void BeginSynchronized()
	{
		m_externalMutex->Acquire();
		++m_nLockCount;
	}

	GE_INLINE void EndSynchronized()
	{
...
public:
	...
	GE_INLINE void Signal()
	{
		pthread_cond_signal(&cond);
	}
	GE_INLINE void Broadcast()
	{
	...
	GE_INLINE bool Wait(time_t ms)
	{
		struct timeval tv;
		gettimeofday(&tv, NULL);
		int64 nsec = tv.tv_usec*1000 + (ms % 1000) * 1000000;

		timespec outtime;
		outtime.tv_sec = tv.tv_sec + nsec / 1000000000 + ms / 1000;
		outtime.tv_nsec = nsec % 1000000000;
		if(pthread_cond_timedwait(&cond, &mut->mutex, &outtime) == 0)
			return true;
		else
			return false;
	}
	...
	{
		mut->Acquire();
//		sLog.Debug("test","lock  %p", pthread_self());
	}
	GE_INLINE void EndSynchronized()
	{
//		sLog.Debug("test","unlock %p", pthread_self());
		mut->Release();
	}
private:
	pthread_cond_t cond;
