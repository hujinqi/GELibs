...
public:
	...
	const static size_t DEFAULT_SIZE = 0x10000;	//65536
...
//	}
	...
	GE_INLINE size_t size() const { return _storage.size(); };
	GE_INLINE size_t max_size() const { return _storage.capacity(); };
	...
	{
		uint32 j = 1, k = 1;
//		sLog.Notice("ByteBuffer","STORAGE_SIZE: %u", (unsigned int)size() );
//		LOG(INFO) << "STORAGE_SIZE: " << (unsigned int)size() << std::endl;
